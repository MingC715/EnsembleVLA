...
class RobotRunner(BaseRunner):
    ...
    def prepare_data(self, policy: BasePolicy, observaton=None) -> bool:
        """
        Prepare inference data for diffusion-level composition (GPC-style).
        Mirrors the implementation from GPC's RobotRunner.
        Note: This will append the observation to the queue.
        """
        if observaton is None:
    ...
    def prepare_data_no_append(self, policy: BasePolicy):
        """
        Prepare inference data without appending observation.
        Use this when observation has already been added via update_obs().
        """
        assert len(self.obs) > 0, "no observation in queue, call update_obs() first"
        device, dtype = policy.device, policy.dtype
        obs = self.get_n_steps_obs()
