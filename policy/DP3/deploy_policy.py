...
def get_model(usr_args):
    ...
    cfg_name = usr_args.get('config_name', 'robot_dp3')
    config_name = f"{cfg_name}.yaml"

    with initialize(config_path=config_path, version_base='1.2'):
        cfg = compose(config_name=config_name)

    now = datetime.now()
    run_dir = f"data/outputs/{now:%Y.%m.%d}/{now:%H.%M.%S}_{cfg_name}_{usr_args['task_name']}"
    ...
    cfg.policy.use_pc_color = usr_args.get('use_rgb', False)
