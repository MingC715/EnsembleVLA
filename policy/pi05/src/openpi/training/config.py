...
class DataConfig:
    ...
    # If true, will only use local files and not download from HuggingFace Hub.
...
class TrainConfig:
    ...
    hub_logging_enabled: bool = False
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotAlohaDataConfig(
            repo_id="demo_clean_repo_new",  # your datasets repo_id
            adapt_to_pi=False,
            repack_transforms=_transforms.Group(inputs=[
                _transforms.RepackTransform({
                    "images": {
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader(os.environ.get("PI05_BASE_PARAMS", "policy/pi05/checkpoints/pi05_param/params")),
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(pi05=True),  # Use pi05=True to match pretrained weights
        data=LeRobotAlohaDataConfig(
            repo_id="demo_clean_repo_new",  # your datasets repo_id
            adapt_to_pi=False,
            repack_transforms=_transforms.Group(inputs=[
        ...
        batch_size=8,  # the total batch_size not pre_gpu batch_size (4 per GPU to avoid OOM)
        #weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        weight_loader=weight_loaders.CheckpointWeightLoader(os.environ.get("PI05_BASE_PARAMS", "policy/pi05/checkpoints/pi05_param/params")),
        num_train_steps=30000,
        fsdp_devices=2,  # Using GPU 2 and 3
    ),
    # pi0_base by full for PyTorch (pi0, NOT pi0.5)
    # Key difference: pi0 uses Pi0Config() without pi05=True
    # pi0.5 uses Pi0Config(pi05=True)
    TrainConfig(
        name="pi0_base_aloha_robotwin_full_pytorch_pi0",
        model=pi0_config.Pi0Config(),  # pi0 (NOT pi0.5, no pi05=True)
        data=LeRobotAlohaDataConfig(
            repo_id="demo_clean_repo_new",  # your datasets repo_id
            adapt_to_pi=False,
            repack_transforms=_transforms.Group(inputs=[
