...
class MlpBlock(nn.Module):
    ...
    def __call__(self, x, deterministic=True):  # noqa: FBT002
        ...
        # Convert string dtype to actual JAX dtype if needed
        dtype_mm = getattr(jnp, self.dtype_mm) if isinstance(self.dtype_mm, str) else self.dtype_mm
        ...
        _, _, d = x.shape  # n,l,d
        x = nn.Dense(self.mlp_dim or 4 * d, dtype=dtype_mm, **inits)(x)
        x = nn.gelu(x)
        x = nn.Dropout(rate=self.dropout)(x, deterministic)
        return nn.Dense(d, dtype=dtype_mm, **inits)(x)
...
class Encoder1DBlock(nn.Module):
    ...
    def __call__(self, x, deterministic=True):  # noqa: FBT002
        ...
        y = nn.LayerNorm(dtype=dtype_mm)(x)
...
class Encoder(nn.Module):
    ...
    def __call__(self, x, deterministic=True):  # noqa: FBT002
        ...
        return nn.LayerNorm(name="encoder_norm", dtype=dtype_mm)(x), out
...
class MAPHead(nn.Module):
    ...
    def __call__(self, x):
        ...
        y = nn.LayerNorm(dtype=dtype_mm)(x)
        x = x + MlpBlock(mlp_dim=self.mlp_dim, dtype_mm=dtype_mm)(y)
        return x[:, 0]
...
class _Module(nn.Module):
    ...
    def __call__(self, image, *, train=False):
        ...
        if self.rep_size:
            ...
            hid = nn.Dense(rep_size, dtype=dtype_mm, name="pre_logits")
        ...
        if self.num_classes:
            ...
            head = nn.Dense(self.num_classes, dtype=dtype_mm, name="head", **kw)
