...
try:
    if os.environ.get("FORCE_MPLIB", "0") == "1":
        raise ImportError("FORCE_MPLIB=1: skipping CuroboPlanner import")
...
class MplibPlanner:
    ...
    def plan_batch(
        ...
        """Sequential batch-planning fallback for mplib planner.

        CuroboPlanner exposes plan_batch; expert-demo validation can fall back to
        MplibPlanner after curobo import/warmup issues, so provide the same basic
        interface by planning each pose independently with plan_path.
        """
        statuses = []
        positions = []
        velocities = []
        for target_pose in target_gripper_pose_list:
            ...
            positions.append(result.get("position", None))
            velocities.append(result.get("velocity", None))
        return {
