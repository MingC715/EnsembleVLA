...
class Module(nn.Module):
    ...
    def embed(self, tokens: at.Int[at.Array, "b t"]) -> at.Float[at.Array, "b t d"]:
        ...
        if isinstance(self.embed_dtype, str):
            embed_dtype = getattr(jnp, self.embed_dtype)
        else:
            embed_dtype = self.embed_dtype
        return self.embedder.encode(tokens).astype(embed_dtype)
    ...
    def __call__(
        ...
        *,
        ...
        if isinstance(self.embed_dtype, str):
            ...
            embed_dtype = self.embed_dtype
        embedded = jax.tree.map(lambda e: e.astype(embed_dtype), embedded)
        ...
        # Convert string dtype to actual JAX dtype if needed for assertion
        if isinstance(self.embed_dtype, str):
            embed_dtype = getattr(jnp, self.embed_dtype)
        else:
            embed_dtype = self.embed_dtype
        assert all(e.dtype == jnp.dtype(embed_dtype) for e in embedded if e is not None)
