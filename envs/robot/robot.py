...
# Try to import CuroboPlanner, set to None if curobo is not installed
try:
    from .planner import CuroboPlanner
except ImportError:
    CuroboPlanner = None
    print("[robot.py]: CuroboPlanner not available. Will use MplibPlanner as fallback.")
...
class Robot:
    ...
    def reset(self, scene, need_topp=False, **kwargs):
        ...
        if self.communication_flag:
            ...
            planner_missing = not hasattr(self, "left_planner") or not hasattr(self, "right_planner")
            planner_is_not_curobo = (
                CuroboPlanner is None
                or planner_missing
                or not isinstance(self.left_planner, CuroboPlanner)
                or not isinstance(self.right_planner, CuroboPlanner)
            )
            if planner_is_not_curobo:
